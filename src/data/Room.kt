package com.sdzsn3.data

import com.sdzsn3.data.models.*
import com.sdzsn3.gson
import com.sdzsn3.other.getRandomWords
import com.sdzsn3.other.matchesWord
import com.sdzsn3.other.transformToUnderscores
import com.sdzsn3.other.words
...
class Room(
    ...
    private var phaseChangesListener: ((Phase) -> Unit)? = null
    var phase = Phase.WAITING_FOR_PLAYERS
        set(value) {
            synchronized(field) {
                field = value
                phaseChangesListener?.invoke(value)
            }
        }

    private fun setPhaseChangedListener(listener: (Phase) -> Unit) {
        phaseChangesListener = listener
    }
    ...
    private suspend fun sendCurRoundInfoToPlayer(player: Player) {
    ...
    suspend fun addPLayer(clientId: String, username: String, socket: WebSocketSession): Player {
        ...
        sendCurRoundInfoToPlayer(player)
    ...
    private fun gameRunning() {
        ...
        val wordWithUnderscored = wordsToSend.transformToUnderscores()
        val drawingUsername = (drawingPlayer ?: players.random()).username
        val gameStateForDrawingPLayer = GameState(
            drawingUsername, wordsToSend
        )

        val gameStateForGuessingPlayers = GameState(
            drawingUsername, wordWithUnderscored
        )
        GlobalScope.launch {
